...
$(document).ready( function($) {
  ...
  fnAjaxLoadMoreNews();
  if($('#about_scene').length>0){
    genLogoAboutParllax();
  }

  if($('#hero_background').length>0){
    genHeroSlideParllax();
  }
  //TYPED JS
  if ($('.element').length) {
...
var $container = $('.masonry');
          ...
          if(data.respond){
            ...
            $.map(data.post, function(obj, i) {
              var html = '<div class="col-lg-4 wow fadeInUp">';
                ...
                html += '<div class="image" style="background-image:url('+obj.image+')"></div>';
                html += '<div class="date">';
                html += '<h3 class="big">'+obj.date+'</h3>';
                html += '<span class="little">'+obj.monthYear+'</span>';
                html += '</div>';
                html += '<div class="category">'+obj.terms[0].name+'</div>';
  ...
  function genLogoAboutParllax(){
    var scene = document.getElementById('about_scene');
  ...
  function genHeroSlideParllax(){
    var scene = document.getElementById('hero_background');
    var parallaxInstance = new Parallax(scene, {
      relativeInput: true
    });
    parallaxInstance.friction(0.2, 0.2);
